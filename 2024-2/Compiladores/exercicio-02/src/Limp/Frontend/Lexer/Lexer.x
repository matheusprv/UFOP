...
module Limp.Frontend.Lexer.Lexer (limpLexer) where
...
mkComment :: AlexPosn -> String -> Token
mkComment p s = Token (position p) (TComment s)

simpleToken :: Lexeme -> AlexPosn -> String -> Token
simpleToken lx p _ = Token (position p) lx

limpLexer :: String -> [Token]
limpLexer = alexScanTokens
}
