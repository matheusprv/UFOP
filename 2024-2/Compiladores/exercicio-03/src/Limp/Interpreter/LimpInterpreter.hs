module Limp.Interpreter.LimpInterpreter (limpInterpProgram) where
...
interpBlock env (Block itens)
  = do
       (env1, vs) <- interpList env $ filterItemStmt itens
...
limpInterpProgram :: Program -> IO Env
limpInterpProgram (Program blk)
  = fst <$> interpBlock [] blk
