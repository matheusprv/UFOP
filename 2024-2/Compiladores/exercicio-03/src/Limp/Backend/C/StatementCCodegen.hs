module Limp.Backend.C.StatementCCodegen where

import Limp.Syntax.Syntax
import Limp.Backend.C.ExpCCodegen
...
programCodegen (Program (Block blk))
  = header <> blks
    where 
      ...
      ret = nest 3 "return 0;"
      stmts = map (blkCodeGen 3) blk ++ [ret, "}"] 
      blks = unlines stmts
...
blockCodegen d (Block stmts) 
  = "{\n" <> ms <> nest (d - 3) "}" 
    where 
      ms = unlines (map (stmtCodegen d) (filterItemStmt stmts))

-- Block code generation

blkCodeGen :: Level -> Item -> String
blkCodeGen d (ItemStmt stmt) = stmtCodegen d stmt
blkCodeGen d (Comment cmmt) = commentCodegen d cmmt

-- Commentary code gen
commentCodegen :: Level -> String -> String
commentCodegen d cmmt = nest d $ "// " ++ cmmt
