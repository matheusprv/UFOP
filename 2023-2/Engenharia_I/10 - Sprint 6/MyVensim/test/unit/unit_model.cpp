...
void unit_Model_createModel(){
    ...
    int amountModels = Model::modelsSize(); 

    Model& model1 = Model::createModel("Modelo 1");
    assert(Model::modelsSize() > amountModels);
    amountModels = Model::modelsSize();  
...
void unit_Model_modelsBegin(){
    ...
    Model::modelsIterator it = model2.modelsBegin();

    assert((*it)->getName() == model1.getName());
...
void unit_Model_modelsSize(){
    int inital_size = Model::modelsSize();

    Model & model1 = Model::createModel("Modelo1");
    Model & model2 = Model::createModel("Modelo2");

    assert(inital_size + 2 == Model::modelsSize());
