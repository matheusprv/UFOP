...
/*
    RESPOSTAS:
    a) Sendo declarada como static, a variavel sempre irá manter o seu valor, mesmo
    após a função ser encerrada e chamada de novo

    b) O sufixo está sendo declarado e tendo um espaço de memória reservado para ele.
    Caso fosse declarado como "char*", o sistema não saberia o quanto de memória alocar
    para a variável. Dessa forma, não seria possível realizar a conversão

    c) O strlen retorna somente a quantidade de caracteres de uma string. Dessa forma, somando
    mais um no "size" é uma forma de incluir o caractere final de uma string, o '\0'

    d) Criar a variavel como estatica e local permite que outros métodos do sistema também 
    possuam uma variável cont sem que ocorra uma interferencia entre os dados por causa de
    variaceis com o mesmo nome

    e) A alocação de memória é necessária para que o comando strcpy copie os dados corretamente
    para o endereço de memória apontado por "str". Caso contrário, teríamos um ponteiro que aponta
    para um local de memória que não terá um comportamento definido, podendo resultar em Segmentation
    Fault.
*/
...
/*
    RESPOSTA: O consumo de memória aumenta pois temos 3 * 10^6 instancias de objetos 
    Para resolver o problema, basta inseirir, na classe cliente, um método destrutor 
    que irá desalocar a memória assim que o objeto for destruido, o que ocorre a cada 
    iteração do for mais interno.
*/

unsigned int Veiculo::cont = 0;
int main(int argc, char *argv[])
{
    const char* const PREFIXO = "Dr. Fulano";
    cout << "espere..." << endl;
...
/*
    RESPOSTA: A linha cli2=cli1 está realizando uma cópia raza, o que pode ocasionar
    problemas, pois o dado não está sendo de fato copiado de uma região da memória para 
    outra. O que realmente está acontecendo é que as duas variáveis estão apontando para
    o memso endereço de memória.
    Assim, é necessário realizar uma cópia profunda dos dados para evitar esse erro, a 
    partir da sobrecarga do operador "=".
*/
...
/*
    RESPOSTA:
    A vantagem do uso da palavra static é, além de poder declar outra variável como "cont",
    o fato de que sempre teremos, o valor mais atual para as novas instâncias de objetos.
    Removendo a palvra static, sempre que um novo objeto for instanciado, ele receberá o 
    valor inicial, ao invés do incremento
*/
...
/*
    RESPOSTA: Atributos estaticos não dependem exclusivamente de uma instância de um objeto,
    pois seu valor não está referenciado exclusivamente por um objeto, ou seja, ele é, de certa
    forma, independente.
*/
...
// sa�da.
/*
    RESPOSTA: Foi iniciado um vetor de Funcionario, onde nele foram adicionados 
    objetos dos tipos Mecanico, GerenteGeral e Gerente. Dessa forma, temos o 
    polimorfismo no código.
    Todavia, todas as subclasses de Funcionario implementam, de forma diferente,
    o método reajustaSalario, cada um de uma maneira diferente. Assim, caso não
    houvesse a palavra virtual, por estarmos utilizando uma variavel do tipo da 
    super classe, o seu método seria chamado. Mas com o virtual, o que é executado
    é o código específico de cada classe.
*/
unsigned int Veiculo::cont = 0;
int main(int argc, char *argv[])
{
...
/*
    RESPOSTAS:
    a) Não é possível instanciar um objeto do tipo veículo, pois a classe é
    abstrata, ou seja, possui métodos que só serão implementados pelas 
    subclasses.

    b) O método foi declarado no arquivo "veiculo.h" como puramente virtual a
    partir da adição do "= 0" no final do mesmo, a fim de indicar que ele não será
    implementado e as subclasses de veículo deverão implementá-la para que não 
    se tornem, também, classes abstratas

    c) 
     - O primeiro const indica que será retornado um ponteiro que não poderá ser modificado
     - A segunda vez, após os parêntesis, indica que os dados dentro do método não poderão
     ser modificados, a não ser que sejam iniciados dentro do mesmo 
*/
